...
HAPPI Database</font></a>
	...
	*/
	...
	$selqry5 = OCIParse($conn, "select * from nsudhara.new_human_string_interactions where protein_id_a = '$ensemblid1' order by COMBINED_SCORE desc");
	...
	while (OCIFetch($selqry5 ))
	{
		...
		while (OCIFetch($selqry9))
		{
			...
			/* Fetch pubmed ids of interacting proteins and insert into a view prot2_abstracts
			$selqry11 = OCIParse($conn, "select protein_ensemblid from nsudhara.string_identifiers_proteins where uniprot_id = '$prot_display'");
			OCIExecute($selqry11, OCI_DEFAULT);
			while (OCIFetch($selqry11))
			{
				$ensembl_id = OCIResult($selqry11, "PROTEIN_ENSEMBLID");
			}
			
			$viewqry2 = OCIParse($conn, "create or replace view prot2_abstracts as select * from string_human_prot_abstracts where ensembl_proteinid like '$ensembl_id'");
