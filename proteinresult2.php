	...
	<tr>
		...
		<tr>
			...
			<?php //STRUCTURE1"; 
				...
				$stmt1=OCIParse($conn,"select PDBID  from PDB.UNIPDB WHERE UNIPROTID='$protein1'");
								OCIExecute($stmt1,OCI_DEFAULT);
				if (OCIFetch($stmt1))
				{$pdbid1= OCIResult($stmt1,"PDBID");}
				OCIFreeStatement($stmt1);

				//qry attempt
				$stmt1=OCIParse($conn,"select STUCTURE  from PDB.PDBSTR WHERE PDBID='$pdbid1'");

				OCIexecute($stmt1);

				
				while ( OCIFetch($stmt1)) 
					{ $lob1 = OCIResult($stmt1,1);
						$pdbstr1 = $lob1->load();
					}
				...
				$stmt2=OCIParse($conn,"select PDBID  from PDB.UNIPDB WHERE UNIPROTID='$protein2'");
								OCIExecute($stmt2,OCI_DEFAULT);
				if (OCIFetch($stmt2))
				{$pdbid2= OCIResult($stmt2,"PDBID");}
				OCIFreeStatement($stmt2);

				//qry attempt
				$stmt2=OCIParse($conn,"select STUCTURE  from PDB.PDBSTR WHERE PDBID='$pdbid2'");

				OCIexecute($stmt2);

				
				while ( OCIFetch($stmt2)) 
					{ $lob2 = OCIResult($stmt2,1);
						$pdbstr2 = $lob2->load();
					}
				...
				<applet name='jmol' code='JmolApplet' archive='JmolApplet.jar'width='250' 
			...
			<applet name='jmol' code='JmolApplet' archive='JmolApplet.jar'width='250' 
					...
					<param name='load'    value='pdbb.pdb'>
					<param name='progressbar' value='true' /> 
					<param name='bgcolor' value='navy'/>
					<param name='script' value='cartoons; color cartoon yellow; spin on' />
			</td>
